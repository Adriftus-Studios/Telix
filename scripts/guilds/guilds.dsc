...
guild_events:
  ...
  events:
    ...
    - if <server.list_notables[inventories].parse[script_name].contains[flag_<player.flag[guild]||null>_<context.location||null>]||false>:
...
guild_gui_events:
  ...
  events:
    ...
    - while <context.inventory.slot[44].material.name||air> == air:
      ...
      - if <[guild]> != null && <yaml[<[guild]>].read[leader].as_player.name||null> != null && <yaml[<[guild]>].read[leader].as_player.flag[guild]||null> == <[guild]>:
