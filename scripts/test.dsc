ability_test_spell:
  ...
  script:
    ...
    - repeat 5:
      ...
      - run ability_test_spell_animation def:<player.eye_location.forward[2]>|<player.eye_location.forward[<script.yaml_key[range]>].up[<[offset].get[1]>].right[<[offset].get[2]>]>|<util.random.int[0].to[360]>
      - wait 2t
