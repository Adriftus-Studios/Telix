ability_group_teleport:
  ...
  points_to_unlock: 1
  power_cost: 20
  description: Teleports all players in the bubble to spawn.
  usage: Use nodestones to teleport players to saved locations.
  icon:
  ...
  script:
    - if <player.location.world.name> != spawn:
      ...
      - else:
        - narrate "<&c>You cannot use this ability for <player.flag[group_teleport_cooldown].expiration.formatted>."
    - else:
      - narrate "<&c>You cannot use this ability in spawn."
