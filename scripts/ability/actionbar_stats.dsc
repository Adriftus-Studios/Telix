...
actionbar_stats_update:
  type: task
  script:
    - actionbar "<&c>Health<&co> <player.health><&fs><player.max_health> <&e>------ <&b>Power<&co> <yaml[player.<player>].read[stats.power.current]><&fs><yaml[player.<player>].read[stats.power.max]>" targets:<player>