...
abilities_characterAbilityTrees:
  ...
  procedural items:
    - foreach <script[ability_trees].list_keys[skill_trees].alphabetical> as:value1:
      - if <script[ability_trees].yaml_key[skill_trees.<[value1]>.available_check].parsed>:
        - define list:|:<script[ability_trees].yaml_key[skill_trees.<[value1]>.GUIItem].as_item.with[display_name=<&b><[value1]>;custom_model_data=<script[ability_trees].yaml_key[skill_trees.<[value1]>.custom_model_data]>;nbt=skillname/<[value1]>]>
    - determine <[list]>
  slots:
