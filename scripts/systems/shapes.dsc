  ...
  - if <player.has_flag[cosmetic]>:
    - if <context.args.get[1]> == test:
      ...
      - while <player.has_flag[test]||false>:
        - rotate <[left_wing]> yaw:<player.location.yaw.to_radians.add[<element[30].to_radians>]> duration:1s frequency:1s
        - rotate <[right_wing]> yaw:<player.location.yaw.to_radians.sub[<element[30].to_radians>]> duration:1s frequency:1s
        - wait 1t
