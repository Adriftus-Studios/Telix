...
build_item:
  ...
  script:
    ...
    - foreach <[item].script.list_keys[equipment_modifiers]> as:modifier:
      - define value:<[item].nbt[base_stats_<[modifier]>]||<[item].script.yaml_key[equipment_modifiers.<[modifier]>]>>
      - if <[item].nbt[base_stats_<[modifier]>]||null> == null:
      ...
      - adjust def:item nbt:base_stats_<[modifier]>/<[value]>
