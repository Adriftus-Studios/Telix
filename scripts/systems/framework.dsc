          ...
          - if <[value].yaml_key[type]> == item:
              ...
              - if <[value].yaml_key[recipes]||null> != null:
                ...
                - foreach <[value].list_keys[recipes]> as:recipe:
                  - yaml id:server.recipe_fixer set restricted.<[value].name>_<[recipe]>:<[value].yaml_key[recipes.<[recipe]>.input]>
...
custom_item_override:
  ...
  events:
    ...
    on item recipe formed:
      - narrate <context.recipe.parse[script.name.to_lowercase.replace[null].with[air]]>
