          ...
          - foreach <[slotmap]> as:slot:
            - if <[slot].split[/].get[2].starts_with[in]>:
              ...
              - if <[contents].map_get[<[item]>]||null> != null:
                - if <[item]||null> != null && <[item].material.name> != air:
          ...
          - if <[crafting]||null> != null && <[crafting]> != air:
            ...
            - if <[time]> > 0:
              - if <[clock]||null> == null:
                ...
                - if <[time]> > 60:
                  - inventory set d:<[inventory]> slot:41 o:<item[alchemy_station_timer].with[display_name=<&7>Cooking<&sp><item[<[crafting]>].script.yaml_key[display<&sp>name].parsed>;quantity=<[time]>;nbt=time/<[time].div[60]>;nbt=crafting/<[crafting]>;lore=<&f><[time].div[60].round_up><&sp>Minutes]>
                - else:
                  - inventory set d:<[inventory]> slot:41 o:<item[alchemy_station_timer].with[display_name=<&7>Cooking<&sp><item[<[crafting]>].script.yaml_key[display<&sp>name].parsed>;quantity=<[time]>;nbt=time/<[time]>;nbt=crafting/<[crafting]>;lore=<&f><[time].round_up><&sp>Seconds]>
              - else:
                - if <[time].ends_with[s]>:
                  - define time:<[time].replace[s].with[]>
                - if <[time].ends_with[m]>:
