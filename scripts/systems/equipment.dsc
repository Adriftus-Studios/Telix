...
equipment_inventory_handler:
  ...
  events:
    ...
    on player clicks in equipment_character with item:
      - define slotmap:<list[11/necklace|12/earrings|16/hat|20/ring1|21/ring2|24/gloves|25/shirt|26/cape|29/trinket1|30/trinket2|34/pants|43/shoes]>
      - foreach <list[hat|gloves|shirt|shoes|pants|cape]>:
        - if <player.open_inventory.slot[<[slotmap].map_find_key[<context.item.script.yaml_key[category]>]>].script.yaml_key[category]||null> != null:
          - if <player.open_inventory.slot[<[slotmap].map_find_key[<context.item.script.yaml_key[category]>]>].script.name.ends_with[_shadow]>:
