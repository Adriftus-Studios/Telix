...
show_recipe:
  ...
  script:
    ...
    - if <[type]> == altar:
      ...
      - inventory set d:<[inv]> slot:1 o:<item[altar_tier_<yaml[server.recipe_book].read[altar.<[item]>.required_tier]>]>
      - narrate <item[altar_tier_<yaml[server.recipe_book].read[altar.<[item]>.tier]>]>
