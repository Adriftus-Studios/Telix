...
show_recipe:
  ...
  script:
    ...
    - if <[type]> == mob_drops:
      ...
      - inventory set d:<[inv]> slot:13 o:<item[gold_nugget].with[display_name=Amount<&sp>dropped;lore=<&r>Max<&sp>Amount:<&sp><entity[<yaml[server.recipe_book].read[mob_drops.<[item]>.max_quantity]>|<&r>Min<&sp>Amount:<&sp><entity[<yaml[server.recipe_book].read[mob_drops.<[item]>.min_quantity]>]>
    - if <[type]> == ore_spawn:
