recipe_book_inventory:
  ...
  - repeat 45:
    - if <[items].size> >= <[value].add[<[page].mul[44].sub[44]>]>:
      - define list:|:<item[<[items].get[<[value].add[<[page].mul[44].sub[44]>]>].split[/].get[1]||air>].as_item||<item[air]>>
    - else:
...
show_recipe:
  ...
  script:
    ...
    - if <[type]> == ore_spawn:
      ...
      - inventory set d:<[inv]> slot:11 o:<item[stone].with[display_name=<&r><&6>Biome<&co>;lore=<[biome_lore]>]>
