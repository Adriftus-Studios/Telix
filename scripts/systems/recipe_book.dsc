  ...
  script:
    ...
    - if <[type]> == mob_drops:
      ...
      - foreach <yaml[server.recipe_book].read[mob_drops.<[item]>.dropped_by].as_list> as:mob:
        - define lore:|:<[mob]>
      - inventory set d:<[inv]> slot:12 o:<item[<entity[<yaml[server.recipe_book].read[mob_drops.<[item]>.dropped_by].as_list.get[1]>].script.yaml_key[entity_type]>].with[display_name=<&7>Dropped<&sp>by<&co>;lore=<[lore]>]>
